...
function loginFromSocialCallBack($socialUser) {
    include './database.php'; // Sử dụng PDO

    $stmt = $pdo->prepare("SELECT `id`, `username`, `email`, `fullname` FROM `user` WHERE `email` = :email");
    $stmt->execute(['email' => $socialUser['email']]);
    
    if ($stmt->rowCount() == 0) {
        $stmt = $pdo->prepare("INSERT INTO `user` (`fullname`, `email`, `status`, `created_time`, `last_updated`) VALUES (:fullname, :email, 1, :created_time, :last_updated)");
        $stmt->execute([
